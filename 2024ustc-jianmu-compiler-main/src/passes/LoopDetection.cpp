...
void LoopDetection::discover_loop_and_sub_loops(BasicBlock *bb, BBset &latches,
    ...
    while (!work_list.empty()) { // 当工作表非空时继续处理
        ...
        if (bb_to_loop_.find(bb) == bb_to_loop_.end()) {
             ...
             */
            loop -> add_block(bb);
            bb_to_loop_.insert(std::make_pair(bb, loop));
            for(auto pre_bb : bb -> get_pre_basic_blocks()){
                // auto pre_bb = &pre_bbs;
                work_list.push_back(pre_bb);
            }
        ...
        // TODO-2: 处理已属于其他循环的节点
             ...
             */
            auto bb_sub_loop = bb_to_loop_[bb];
            while (bb_sub_loop->get_parent()) {
                bb_sub_loop = bb_sub_loop -> get_parent();
            }
            if (bb_sub_loop == loop)continue;
            bb_sub_loop -> set_parent(loop);
            loop -> add_sub_loop(bb_sub_loop);
            for (auto sub_block : bb_sub_loop -> get_blocks()) {
                loop -> add_block(sub_block);
                bb_to_loop_[sub_block] = loop;
            }

            for (auto header_pred : bb_sub_loop -> get_header() -> get_pre_basic_blocks()) {
                work_list.push_back(header_pred);
            }
        }             
    }
}
