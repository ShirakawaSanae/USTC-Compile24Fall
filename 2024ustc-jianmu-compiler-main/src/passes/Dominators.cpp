...
void Dominators::run_on_func(Function *f) {
   // LOG(DEBUG) << "Dominators run on func\n";
...
void Dominators::create_reverse_post_order(Function *f) {
    // LOG(DEBUG) << "Dominators create_reverse_post_order\n";
    BBSet visited;
...
void Dominators::create_idom(Function *f) {
    // LOG(DEBUG) << "Dominators create idom\n";
    // TODO: 分析得到 f 中各个基本块的 idom
    // throw "Unimplemented create_idom";
    for(auto &bb : f -> get_basic_blocks()) set_idom(&bb, nullptr);
    auto f_entry = f -> get_entry_block();
    idom_[f_entry] = f_entry;
    bool changed = true;
    while(changed){
        changed = false;
        for(auto ite = post_order_vec_.rbegin(); ite != post_order_vec_.rend(); ite++){
            auto bb = *(ite);
            if(bb == f_entry) continue;
            auto pre_bb_list = bb -> get_pre_basic_blocks();
            BasicBlock *new_idom = nullptr;
            for(auto pre_bb : pre_bb_list){
                if(get_idom(pre_bb) != nullptr){
                    if(new_idom == nullptr) new_idom = pre_bb;
                    else new_idom = intersect(new_idom, pre_bb);
                }
            }
...
void Dominators::create_dom_tree_succ(Function *f) {
    // LOG(DEBUG) << "Dominators create dom_tree_succ\n";
    // TODO: 分析得到 f 中各个基本块的支配树后继
    // direct successor, no recursion
    // throw "Unimplemented create_dom_tree_succ";
    for(auto &bbs : f -> get_basic_blocks()){
        auto bb = &bbs;
        auto bb_idom = get_idom(bb);
        if(bb_idom != nullptr && bb != f -> get_entry_block()){
            /*if(dom_tree_succ_blocks_.find(bb) == dom_tree_succ_blocks_.end()){
                std::set<BasicBlock *> succ_set;
                dom_tree_succ_blocks_.insert(std::make_pair(bb_idom, succ_set));
            }*/
            add_dom_tree_succ_block(bb_idom, bb);
        }
    }
...
void Dominators::create_dom_dfs_order(Function *f) {
    // LOG(DEBUG) << "Dominators create_dom_dfs_order\n";
