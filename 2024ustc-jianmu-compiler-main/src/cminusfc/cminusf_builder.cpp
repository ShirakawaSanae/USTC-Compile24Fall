...
Value* CminusfBuilder::visit(ASTParam &node) {
    ...
    // 根据类型判断
    if (node.type == TYPE_INT) {
        if (node.isarray) param_alloc = builder -> create_alloca(INT32PTR_T);
        else param_alloc = builder -> create_alloca(INT32_T);
    } else if (node.type == TYPE_FLOAT) {
        if (node.isarray) param_alloc = builder -> create_alloca(FLOATPTR_T);
        else param_alloc = builder -> create_alloca(FLOAT_T);
    }
    scope.push(node.id, param_alloc);
...
Value* CminusfBuilder::visit(ASTSelectionStmt &node) {
    ...
    Value *cond_value = context.global_val;

    BasicBlock *trueBB = BasicBlock::create(module.get(), "", context.global_func);
    // 准备一个 falseBB 以防有 else
    BasicBlock *falseBB = node.else_statement ? BasicBlock::create(module.get(), "", context.global_func) : nullptr;
    BasicBlock *contBB = BasicBlock::create(module.get(), "", context.global_func);

    Value *cond;
    if (cond_value -> get_type() -> is_integer_type()) {
        cond = builder -> create_icmp_ne(cond_value, CONST_INT(0));
    } else {
        cond = builder -> create_fcmp_ne(cond_value, CONST_FP(0.));
    }

    if(node.else_statement) builder -> create_cond_br(cond, trueBB, falseBB);
    else builder -> create_cond_br(cond, trueBB, contBB);

    // 处理 if 语句
    builder -> set_insert_point(trueBB);
    node.if_statement -> accept(*this); // 这里注意是先 insert 再 accept
    if (not builder -> get_insert_block() -> is_terminated()) {
...
bool check_type(std::unique_ptr<IRBuilder> &builder, Value **l_val_p, Value **r_val_p) {
    ...
    if(l_val -> get_type() == r_val -> get_type()) {
        return l_val -> get_type() -> is_integer_type();
    }
    else {
...
Value* CminusfBuilder::visit(ASTVar &node) {
    ...
    bool lvalue = context.require_lvalue;
    context.require_lvalue = false;
    // ... = expression ...
    if(node.expression == nullptr){ // 直接返回变量值 或者 索引
        if(lvalue) context.global_val = var_;
        else{
        ...
        if (lvalue) context.global_val = tmp_ptr; 
        else context.global_val = builder -> create_load(tmp_ptr);
    }
    //context.require_lvalue = false;
    return nullptr;
}
