...
void translate_main(CodeGen *codegen) {
    ...
    // - 数组的大小是多少字节?
    // - 数组的偏移量是多少字节?
    offset_map["%op0"] = -24;  // TODO: 请填空 16 + 8
    offset_map["array"] = -64; // TODO: 请填空 24 + 4 * 10
    ...
    // 你需要思考 %op1 的偏移量是多少字节
    offset_map["%op1"] = -72; // TODO: 请填空 64 + 8
    // 获得数组的地址, 将其写入 $t0 中
    ...
    // 你可以手动计算 gep 的值, 或者使用指令计算 gep 的值
    // 你需要思考 %op2 的偏移量是多少字节
    offset_map["%op2"] = -80; // TODO: 请填空 72 + 8
    // 获得数组的地址, 将其写入 $t0 中
    codegen->append_inst("ld.d",
                         {"$t0", "$fp", std::to_string(offset_map["%op0"])});
    ...
    codegen->append_inst("ld.w", {"$t0", "$fp", std::to_string(offset_map["%op4"])});
    codegen->append_inst("ld.d", {"$t1", "$fp", std::to_string(offset_map["%op2"])});
    ...
    offset_map["%op5"] = -92; // TODO: 请填空 88 + 4
    // TODO: 先获得 %op2 的值, 然后获得 %op2 指向的空间的值, 最后将这个值写入
    // %op5 对应的内存空间中
    codegen->append_inst("ld.d", {"$t1", "$fp", std::to_string(offset_map["%op2"])});
    codegen->append_inst("ld.w $t1, $t1, 0");
    codegen->append_inst("st.w", {"$t1", "$fp", std::to_string(offset_map["%op5"])});    

    /* ret i32 %op5 */
    codegen->append_inst("ret i32 %op5", ASMInstruction::Comment);
    // TODO: 将 %op5 的值写入 $a0 中
    codegen->append_inst("ld.w", {"$a0", "$fp", std::to_string(offset_map["%op5"])});
